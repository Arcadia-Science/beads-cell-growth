...
def violin_grouped_with_means(df: pd.DataFrame, y: str, title: str, outpath: Path) -> None:
    ...
    for strain in STRAIN_ORDER:
        ...
        try:
            ...
            # Dunnett test vs NO bead control
            try:
                from statsmodels.stats.multicomp import MultiComparison
                mc = MultiComparison(sdf[y], sdf['treatment'])
                dunnett_res = mc.allpairtest(stats.ttest_ind, method='dunnett')
                dunnett_table = dunnett_res[1]
                print(f"Dunnett test results for {strain}:")
                print(dunnett_table)
            except Exception as e:
                print(f"Dunnett test failed for {strain}: {e}")
