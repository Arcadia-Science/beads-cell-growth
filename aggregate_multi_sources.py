...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "strain" not in df2.columns:
        df2["strain"] = "unknown"
    group_cols = ["experiment", "strain", "bead_present", "volume_ml"] if "experiment" in df2.columns else ["strain", "bead_present", "volume_ml"]
        ...
        for idx, (mean_col, title, stdev_col) in enumerate(facets):
            ...
            for exp in experiments:
                for bead in bead_labels:
                    d = df_strain[(df_strain['experiment']==exp) & (df_strain['bead_present']==bead)]
                    d = d[d['volume_ml'].isin(x_volumes)]
                    if mean_col in d.columns and stdev_col in d.columns and not d.empty:
                        ax.errorbar(d['volume_ml'], d[mean_col], yerr=d[stdev_col], marker='o', label=f'{exp} ({bead_names[bead]})', capsize=4)
            ax.set_title(title)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python aggregate_multi_sources.py <processed_folder1> <processed_folder2> ... <output_csv>")
        sys.exit(1)
    *input_folders, output_csv = sys.argv[1:]
        ...
        for csv_file in folder_path.glob("*.csv"):
            ...
            if experiment == "96-well":
                if "treatment" in df.columns:
                    ...
                    df["bead_present"] = df["bead_present"].astype(str).str.lower().map({"no": False, "1": True, "yes": True, "true": True, "false": False, "0": False})
                df["volume_ml"] = 1.0
            else:
    ...
    od_df["bead_present"] = od_df["bead_present"].astype(str).str.lower().map({"true": True, "false": False, "1": True, "0": False})
