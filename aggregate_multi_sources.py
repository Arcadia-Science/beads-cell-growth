...
def find_combined_csv(base: Path) -> Path | None:
    # check processed/combined_dic_measurements.csv or combined_dic_measurements.csv
...
# Attempt to use arcadia-pycolor palette when available. We provide a small
# helper to fetch a named color and fall back to a hard-coded hex if missing.
try:
...
def _arc_hex(name: str, fallback: str) -> str:
    ...
    name is the attribute name on the arcadia_pycolor module (e.g. 'canary').
    fallback is the hex string to use if arcadia isn't available or the name
    isn't present.
    """
    if ap is None:
        return fallback
...
try:
    ...
    # also set base font size from arcadia defaults if available
    if ap is not None:
        try:
...
def load_source(path: Path, label: str) -> pd.DataFrame:
    ...
    if p is None:
        raise FileNotFoundError(f"combined_dic_measurements.csv not found under {path} or {path}/processed")
    df = pd.read_csv(p)
    df["__source"] = label
    return df
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = g.agg(length_mean=("length", "mean"), length_sd=("length", "std"), length_n=("length", "count"),
                area_mean=("area", "mean"), area_sd=("area", "std"), area_n=("area", "count")).reset_index()
    return out


def plot_strain_facets(df_summary: pd.DataFrame, strain: str, outpath: Path) -> None:
    if sns is None:
        raise RuntimeError("seaborn is required for plotting. Install with: pip install seaborn")
    ...
    for ax in axes.flat:
        try:
            ...
            pass
    has_od = all(c in df.columns for c in ("OD_morning_mean", "OD_morning_stdev", "OD_afternoon_mean", "OD_afternoon_stdev"))
    ...
    for j_col, phen in enumerate(phenotypes):
        ...
        for src in sources:
            for bead_label in bead_labels:
                ...
                ax.errorbar(x_plot, y, yerr=yerr, fmt=marker_map.get(bead_label, 'o'), color=color, capsize=0, label=label)
        ...
        if has_od:
            for src in sources:
                for bead_label in bead_labels:
                    ...
                    x_plot = x + base_offsets.get(src, 0.0)
                    color = color_map.get((src, bead_label), None)
                    ax_od_m.errorbar(x_plot, y, yerr=yerr, fmt=marker_map.get(bead_label, 'o'), color=color, capsize=0)
            ...
            ax_od_m.text(0.5, 0.5, 'No OD morning data', horizontalalignment='center', verticalalignment='center', transform=ax_od_m.transAxes)
        ...
        if has_od:
            for src in sources:
                for bead_label in bead_labels:
                    ...
                    x_plot = x + base_offsets.get(src, 0.0)
                    color = color_map.get((src, bead_label), None)
                    ax_od_a.errorbar(x_plot, y, yerr=yerr, fmt=marker_map.get(bead_label, 'o'), color=color, capsize=0)
            ...
            ax_od_a.text(0.5, 0.5, 'No OD afternoon data', horizontalalignment='center', verticalalignment='center', transform=ax_od_a.transAxes)
            ax_od_a.set_axis_off()

    import matplotlib.lines as mlines
    # create legend entries with consistent colors by source and markers by bead label
    handles = [mlines.Line2D([], [], color=color_map.get((s, b)), marker=marker_map[b], linestyle='None', label=f"{s} ({b})")
               for s in sources for b in bead_labels]
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Aggregate multiple processed folders and plot mean length/area by volume")
    parser.add_argument("--paths", "-p", nargs=3, required=True, help="Three base folders: 96-well, 24-well, ttubes (in that order)")
    parser.add_argument("--map-96-volume", type=float, default=1.0, help="Volume (mL) to assign to 96-well 4.5 mm bead samples when mapping to the 1..5 mL x-axis")
    parser.add_argument("--out-dir", "-o", default=Path.cwd(), type=Path)
    parser.add_argument("--plots", action="store_true", help="Generate per-strain facetted plots (length, area, OD) into out-dir")
