...
| Path | Contents |
|---|---|
| `notebooks/` | Jupyter notebooks for OD and microscopy analysis, plus helper modules |
| `scripts/` | Cellpose segmentation pipeline and microscopy data compilation scripts |
| `figures/` | SVG figure outputs generated by the analysis notebooks |
...
- Microscopy data analysis is performed in `notebooks/microscopy-analysis.ipynb`, which reads cell morphology measurements from compiled CSVs in `data/microscopy/`.
